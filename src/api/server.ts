...
app.get('/console/entity-detail', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const html = await readFile(
    path.join(process.cwd(), 'preview', 'entity-detail', 'index.html'),
    'utf8'
  );
  return reply.type('text/html; charset=utf-8').send(html);
});

app.get('/console/entity-detail/entity.css', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const css = await readFile(
    path.join(process.cwd(), 'preview', 'entity-detail', 'entity.css'),
    'utf8'
  );
  return reply.type('text/css; charset=utf-8').send(css);
});

app.get('/console/entity-detail/view.mjs', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const moduleSource = await readFile(
    path.join(process.cwd(), 'preview', 'entity-detail', 'view.mjs'),
    'utf8'
  );
  return reply.type('text/javascript; charset=utf-8').send(moduleSource);
});
...
app.get('/console/revision-ledger', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const html = await readFile(
    path.join(process.cwd(), 'preview', 'revision-ledger', 'index.html'),
    'utf8'
  );
  return reply.type('text/html; charset=utf-8').send(html);
});

app.get('/console/revision-ledger/revision.css', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const css = await readFile(
    path.join(process.cwd(), 'preview', 'revision-ledger', 'revision.css'),
    'utf8'
  );
  return reply.type('text/css; charset=utf-8').send(css);
});

app.get('/console/revision-ledger/view.mjs', async (_request, reply) => {
  if (!isLocalMemory) return reply.code(404).send({ code: 'NOT_FOUND' });
  const moduleSource = await readFile(
    path.join(process.cwd(), 'preview', 'revision-ledger', 'view.mjs'),
    'utf8'
  );
  return reply.type('text/javascript; charset=utf-8').send(moduleSource);
});
