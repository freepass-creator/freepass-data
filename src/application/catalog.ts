...
import type {
  ActorRef, ErpPublicProduct, Money, Offer, ProjectionRelease
...
function replacePriceTerm(offer: Offer, termMonths: number, monthlyRent: Money): Offer {
  if (!offer.priceTerms.some((term) => term.termMonths === termMonths)) {
    throw new InvalidCommandError(
      `Offer ${offer.id} has no ${termMonths}-month PriceTerm; adding a condition is a separate command.`
    );
  }
  return {
    ...offer,
    priceTerms: offer.priceTerms.map((term) =>
      term.termMonths === termMonths ? { ...term, monthlyRent } : term
    )
  };
}
...
export async function updateOfferPrice(
  ...
  return store.transact(async (tx) => {
    ...
    const repriced = replacePriceTerm(current, input.termMonths, input.monthlyRent);
    const next: Offer = {
      ...repriced,
    ...
    const receipt = {
      ...
      status: 'CANONICAL_COMMITTED' as const,
      entityType: 'offer',
      entityId: current.id,
    ...
    await tx.appendOutbox({
      ...
      eventType: 'catalog.offer.changed',
      entityType: 'offer',
      entityId: current.id,
...
export async function buildErpPublicProjection(
  ...
  const canonicalRevision = Math.max(
    0,
    ...models.map((x) => x.revision),
    ...assets.map((x) => x.revision),
    ...products.map((x) => x.revision),
    ...offers.map((x) => x.revision)
  );
...
export async function processOneOutboxEvent(
  ...
  outbox: OutboxStore,
  projections: ProjectionStore,
  options: { workerId: string; maxAttempts?: number; baseBackoffMs?: number; leaseMs?: number },
  now = new Date()
): Promise<'IDLE' | 'DONE' | 'RETRY' | 'DEAD_LETTER'> {
  const event = await outbox.claimNext({
    workerId: options.workerId,
    now: now.toISOString(),
    leaseUntil: new Date(now.getTime() + (options.leaseMs ?? 30_000)).toISOString()
  });
  ...
  try {
    ...
    await outbox.markDone(event.eventId);
    return 'DONE';
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (attempts >= (options.maxAttempts ?? 8)) {
      await outbox.moveToDeadLetter({ eventId: event.eventId, attempts, error: message });
      return 'DEAD_LETTER';
    }
    await outbox.markRetry({
      ...
      nextAttemptAt: new Date(
        now.getTime() + backoffMs(options.baseBackoffMs ?? 500, attempts)
      ).toISOString(),
      error: message
    });
    return 'RETRY';
